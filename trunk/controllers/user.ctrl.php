		...
		if(!$this->validate->flagErr){
			if (!$this->__checkUserName($userInfo['userName'])) {
				if (!$this->__checkEmail($userInfo['email'])) {
					$sql = "insert into users(id,utype_id,username,password,first_name,last_name,email,status) 
							values('',2,'{$userInfo['userName']}','".md5($userInfo['password'])."','{$userInfo['firstName']}','{$userInfo['lastName']}','{$userInfo['email']}',1)";
					$this->db->query($sql);
					$this->listUsers('ajax');
					exit;
				}else{
