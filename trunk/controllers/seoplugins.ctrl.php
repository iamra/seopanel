...
class SeoPluginsController extends Controller{
	...
	function manageSeoPlugins($info, $method='get') {
		...
		$pluginDirName = $this->__encodeDirName($pluginInfo['name']);
	...
	function showSeoPlugins($info=''){
		...
		foreach($menuList as $i => $menuInfo){
			...
			$pluginDirName = $this->__encodeDirName($menuInfo['name']);
	...
	function reInstallSeoPlugin($pluginId){
		...
		if(file_exists($pluginDBFile)){
			$this->db->importDatabaseFile($pluginDBFile);
		}		
		...
		if ($handle = opendir(SP_PLUGINPATH)) {
			while (false !== ($file = readdir($handle))) {
				if ( ($file != ".") && ($file != "..") &&  is_dir(SP_PLUGINPATH."/".$file) ) {
					$pluginName = $this->__decodeDirName($file);
					$seoPluginInfo = $this->__getSeoPluginInfo($pluginName, 'name');
					if(empty($seoPluginInfo['id'])){
						$sql = "insert into seoplugins(id,name,label,status,installed) values('','$pluginName','$pluginName',0,1)";
						$this->db->query($sql);
	...
	# func to encode dir name
	function __encodeDirName($dirName){
		$dirName = str_replace(' ', '-', $dirName);
		return $dirName;
	}
	
	# func to decode dir name
	function __decodeDirName($dirName){		
		$dirName = str_replace('-', ' ', $dirName);
		return $dirName;
	}
}
