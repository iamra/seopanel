...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="search">
	<tr>
		...
		<td colspan="2"><a href="javascript:void(0);" onclick="scriptDoLoadPost('directories.php', 'search_form', 'content', '&sec=reports')" class="actionbut"><?php echo $spText['button']['Show Records']?></a></td>
	</tr>
</table>
</form>
		...
		foreach($list as $listInfo){
			...
			<tr class="<?php echo $class?>">
				...
				<td class="<?php echo $rightBotClass?>">
					<select name="action" id="action<?php echo $listInfo['id']?>" onchange="doAction('directories.php', '<?php echo $statusId?>', 'id=<?php echo $listInfo['id']?>', 'action<?php echo $listInfo['id']?>')">
